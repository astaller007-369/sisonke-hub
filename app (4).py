...
# ==============================================================================
    ...
    if os.path.exists(database_csv_path):
        ...
        master_db_df = pd.DataFrame()
    # 🎛️ DROP-DOWN AUTO-POPULATION MATRIX CONTROLS
    ...
    # Render interactive layout grids side-by-side on your web browser viewport pane
    ...
    with check_col:
        ...
        match_state["checklist"]["c12"] = st.checkbox("Check early line news leaks", value=match_state["checklist"]["c12"], key="cb12", help="Check for team news leaks on social channels.")
    ...
    with tracking_col:
        ...
        if checked_count < 8:
            st.error("🛑 PASS / NO BET: Information scarcity detected. Lock more checklist steps to establish data validity.")
        elif auto_confidence >= 7.5:
            st.success("🎯 VALUE TRADING MODE: High verification level matched with solid market sentiment. Proceed to execute manual overrides.")
        else:
            st.warning("🔷 ALTERNATIVE LINE LOCK: Mixed data signals or negative price movements. Target high-safety alternative options.")
